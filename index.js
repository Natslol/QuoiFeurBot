...
client.on('ready', () => {
	...
	client.user.setActivity(`Ludo`, { type: "LISTENING", url: "https://github.com/Ludo1337/QuoiFeurBot/"})
});
        ...
        if (message.content.endsWith(char[a])) {
            ...
            quoi.forEach(i => {
                ...
                if (split[split.length - 1].includes(i)) {
                    ...
                    console.log(`${message.author.tag} (${message.author.id}) c'est pris un peti" feur dans ${message.guild} (${message.guild.id})`)
